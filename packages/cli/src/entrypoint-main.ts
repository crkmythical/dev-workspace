...
if (subUrl) {
  const fetchProc = Bun.spawn(["curl", "-fsSL", "--noproxy", "*", "--max-time", "30", subUrl, "-o", "/etc/clash/config.yaml.new"], {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (selector && selector.now === "🎯 全球直连" && selector.all?.length > 1) {
        ...
        const autoGroup = selector.all.find((n: string) =>
          n.includes("自动") || n.includes("auto") || n.includes("url-test")
        );
        // Otherwise pick first real proxy node
        const proxyNode = autoGroup || selector.all.find((n: string) =>
          !n.includes("直连") && !n.includes("DIRECT") && !n.includes("拦截") &&
          !n.includes("REJECT") && !n.includes("净化") && !n.includes("漏网")
        );
        if (proxyNode) {
          await fetch("http://127.0.0.1:9090/proxies/%F0%9F%9A%80%20%E8%8A%82%E7%82%B9%E9%80%89%E6%8B%A9", {
            method: "PUT",
...
let egress = false;
for (let i = 0; i < 3; i++) {
  const probe = await $`curl -fsSL --proxy socks5://127.0.0.1:${CLASH_SOCKS_PORT} --max-time 10 https://www.google.com -o /dev/null`.quiet().nothrow();
  if (probe.exitCode === 0) { egress = true; break; }
